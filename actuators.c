...
void vActuatorsConfig(void) { for(uint8_t Chan = 0; Chan < NumActuator; xActuatorConfig(Chan++)); }
...
int	xActuatorStartSequence(uint8_t Chan, uint8_t Seq) {
	if (Chan >= NumActuator || sAI[Chan].Blocked || Seq >= actMAX_SEQUENCE)
		return erFAILURE;
